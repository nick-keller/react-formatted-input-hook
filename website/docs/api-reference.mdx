...
The [un-formatted]('./under-the-hood) value that is used in the virtual input.
...
- `value` is the un-formatted value
...
This callback is called when the user blurs the input, and lets you update the [un-formatted]('./under-the-hood) value
if necessary.
...
- `params.key` which is the [*key*](https://developer.mozilla.org/en-US/docs/Web/API/KeyboardEvent/key) of the keyboard event
...
This callback is called when the user tries to paste a value. 
It works exactly like [onKeyDown](#onkeydown), but instead of `params.key` it has `params.clipboard`
which is the string representation of the what the user is trying to paste
...
The length of themapping should be `1 + "1234".length` which is 5. Here the mapping would be `[1, 1, 3, 4, 5]` (notice that we repeated the first value to have a length of 5).
...
Used to insert [un-formatted]('./under-the-hood) text at caret. Caret will automatically be moved to the end on the inserted text, 
...
Used to completely replace the current value with an [un-formatted]('./under-the-hood) text. 
